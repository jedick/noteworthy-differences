...
def classifier_logic():
    ...
    # Test condition is True if models disagree
    classifier_condition = (
        heuristic["noteworthy"] == False and few_shot["noteworthy"] == True
    )
...
def test_classifier():
    """Run classifier logic up to 5 times"""
    current_try = 0
    max_trys = 5
    while current_try < max_trys:
        current_try += 1
        result = classifier_logic()
        if result is True:
